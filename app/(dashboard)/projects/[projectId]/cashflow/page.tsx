...
export default function CashFlowPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const lastCF        = records[records.length - 1]
    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-black tracking-[-0.5px]">Cash Flow</h1>
          <p className="text-sm text-[#6B7280] mt-1">Monthly planned vs actual expenditure tracking</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Total Planned',  value: formatCurrency(totalPlanned, 'SAR'),  color: '#000' },
          ...
          <div key={k.label} className="bg-white rounded-xl px-5 py-4 border border-gray-200">
            <div className="text-[10px] text-[#6B7280] uppercase tracking-wider mb-1">{k.label}</div>
            <div className="text-xl font-bold tracking-[-0.3px]" style={{ color: k.color }}>{k.value}</div>
            {k.sub && <div className="text-[10px] text-[#6B7280] mt-0.5">{k.sub}</div>}
          </div>
        ))}
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <label className="block text-[11px] font-semibold text-[#374151] mb-1.5">Year</label>
            ...
            <div>
              <label className="block text-[11px] font-semibold text-[#374151] mb-1.5">Month</label>
              <select className={inputCls} value={form.month} onChange={e => setForm(f => ({ ...f, month: e.target.value }))}>
                {MONTH_NAMES.map((m, i) => <option key={i+1} value={i+1}>{m}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-[#374151] mb-1.5">Planned (SAR)</label>
            ...
            <div>
              <label className="block text-[11px] font-semibold text-[#374151] mb-1.5">Actual (SAR)</label>
          ...
          <div className="flex gap-3 mt-4">
            ...
            <button type="button" onClick={() => { setShowForm(false); setEditId(null) }} className="text-sm text-[#6B7280] hover:text-black transition-colors">Cancel</button>
          </div>
        </form>
      )}
      ...
      {records.length > 1 && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[13px] font-bold text-black uppercase tracking-wider">Monthly View</h3>
          ...
          <div className="flex items-end gap-2 h-32 overflow-x-auto">
            {records.map(r => {
              ...
              return (
                <div key={r.id} className="flex items-end gap-0.5 flex-shrink-0" title={`${MONTH_NAMES[r.month-1]} ${r.year}: P=${r.planned.toLocaleString()} A=${r.actual.toLocaleString()}`}>
          ...
          <div className="flex gap-2 mt-1 overflow-x-auto">
            {records.map(r => (
              <span key={r.id} className="w-9 flex-shrink-0 text-[9px] text-center text-[#6B7280]">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 border-2 border-dashed border-gray-200 rounded-2xl">
          ...
          <p className="text-sm font-semibold text-black mb-1">No cash flow data yet</p>
          <p className="text-[12px] text-[#6B7280]">Add monthly planned and actual values to track project spend.</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="grid grid-cols-[120px_1fr_1fr_1fr_1fr_1fr_60px] gap-4 px-6 py-3 bg-[#F3F4F6] border-b border-gray-200 text-[10px] font-bold text-[#6B7280] uppercase tracking-wider">
            <span>Month</span>
          ...
          <div className="divide-y divide-gray-50">
            {records.map(r => {
              ...
              return (
                <div key={r.id} className="grid grid-cols-[120px_1fr_1fr_1fr_1fr_1fr_60px] gap-4 items-center px-6 py-3.5 hover:bg-[#F9FAFB] transition-colors text-[12px]">
                  <span className="font-semibold text-black">{MONTH_NAMES[r.month-1]} {r.year}</span>
                  <span className="text-right text-[#374151]">{formatCurrency(r.planned, 'SAR')}</span>
                  <span className="text-right text-[#374151]">{formatCurrency(r.actual,  'SAR')}</span>
                  <span className={`text-right font-semibold ${isNeg ? 'text-orange-600' : 'text-green-600'}`}>
                    {isNeg ? '-' : '+'}{formatCurrency(Math.abs(r.variance), 'SAR')}
                  </span>
                  <span className="text-right text-[#6B7280]">{formatCurrency(r.cumulativePlanned, 'SAR')}</span>
                  <span className="text-right text-[#6B7280]">{formatCurrency(r.cumulativeActual,  'SAR')}</span>
                  <div className="flex gap-2 justify-end">
                    <button onClick={() => openEdit(r)} className="text-[#6B7280] hover:text-black transition-colors">Edit</button>
