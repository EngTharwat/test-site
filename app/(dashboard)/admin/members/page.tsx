...
export default function MembersPage() {
  ...
  if (!profile || profile.role !== 'admin') {
    return (
      <div className="p-8 text-center">
        <p className="text-[#6B7280]">Only admins can manage team members.</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <button onClick={() => router.push('/dashboard')}
            className="text-[12px] text-[#6B7280] hover:text-black mb-1 flex items-center gap-1 transition-colors">
            ← Portfolio
          </button>
          <h1 className="text-2xl font-bold text-black tracking-[-0.5px]">Team Members</h1>
          <p className="text-sm text-[#6B7280] mt-1">
            Portfolio: <strong className="text-black">{profile.portfolioName}</strong>
            <span className="ml-2 text-[11px] bg-gray-100 px-2 py-0.5 rounded font-mono">{profile.portfolioSlug}</span>
          </p>
        </div>
        <button
      ...
      <div className="bg-[#F9FAFB] border border-[#E5E7EB] rounded-xl p-4 mb-6 text-[12px] text-[#374151]">
        <strong>Team login info:</strong> Members go to the login page and enter portfolio name{' '}
        <span className="font-mono bg-white border border-gray-200 px-1.5 py-0.5 rounded">{profile.portfolioSlug}</span>
      ...
      {showForm && (
        <form onSubmit={handleAdd} className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          <h3 className="text-[13px] font-bold text-black uppercase tracking-wider mb-4">New Team Member</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              />
              <p className="text-[10px] text-[#9CA3AF] mt-0.5">Lowercase, no spaces</p>
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-[#374151] mb-1.5">Display Name *</label>
              <input
                className={inputCls} required
            ...
            <div>
              <label className="block text-[11px] font-semibold text-[#374151] mb-1.5">Role *</label>
              <select
                className={selectCls}
                value={form.role}
          ...
          {/* Role description */}
          <p className="text-[11px] text-[#6B7280] mt-3 bg-[#F9FAFB] border border-[#E5E7EB] rounded-lg px-3 py-2">
            🔑 {ROLE_DESCRIPTIONS[form.role]}
          </p>
          {error && <p className="text-[12px] text-red-600 mt-3">{error}</p>}
          <div className="flex gap-3 mt-4">
            <button type="submit" disabled={saving}
              className="bg-black text-white text-sm font-semibold px-5 py-2 rounded-lg hover:bg-[#0F1115] disabled:opacity-50 transition-colors">
      ...
      {error && !showForm && <p className="text-[12px] text-red-600 mb-4">{error}</p>}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="grid grid-cols-[1fr_160px_160px_80px] gap-4 px-6 py-3 bg-[#F3F4F6] border-b border-gray-200 text-[10px] font-bold text-[#6B7280] uppercase tracking-wider">
            <span>Member</span>
            <span>Username</span>
            <span>Role</span>
            <span></span>
          ...
          <div className="divide-y divide-gray-50">
            {members.map(m => (
              <div key={m.username} className="grid grid-cols-[1fr_160px_160px_80px] gap-4 items-center px-6 py-4">
                <div>
                  <div className="text-[13px] font-semibold text-black">{m.displayName}</div>
                  {m.username === 'admin' && (
                    <div className="text-[10px] text-[#9CA3AF]">Portfolio admin</div>
                  )}
                </div>
                <span className="font-mono text-[12px] text-[#374151]">{m.username}</span>
                <div>
                  {m.username === 'admin' ? (
                    ...
                    <select
                      className="text-[11px] font-semibold border border-gray-200 rounded-lg px-2 py-1 bg-white focus:outline-none focus:ring-1 focus:ring-black/10 cursor-pointer"
                      value={m.role}
      ...
      <div className="mt-8 bg-white rounded-xl border border-gray-200 p-6">
        <h3 className="text-[13px] font-bold text-black uppercase tracking-wider mb-4">Role Permissions</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {ROLES.map(r => (
            <div key={r} className="flex items-start gap-3 p-3 rounded-lg bg-[#F9FAFB] border border-[#F3F4F6]">
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap mt-0.5 ${ROLE_COLORS[r]}`}>
                {ROLE_LABELS[r]}
              </span>
              <p className="text-[11px] text-[#6B7280] leading-relaxed">{ROLE_DESCRIPTIONS[r]}</p>
            </div>
          ))}
        </div>
      </div>
